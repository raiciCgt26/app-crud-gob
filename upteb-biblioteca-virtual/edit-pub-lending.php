...
session_start();
$title = "Modificar Prestamo de Publicación";
...
// Sacar detalles del ticket de la base de datos.
$sql_tick = "SELECT * FROM pub_lending WHERE book_act_id = '$plc_id'";
$tick_result = mysqli_query($con, $sql_tick);



// Asegurarse que el libro exista.
/*if (mysqli_num_rows($tick_result) == 0) {
  header("Location: pub-lending-tickets.php");
  exit;
}*/

$ticket_row = mysqli_fetch_assoc($tick_result);



$sql_publications = "SELECT * FROM publications WHERE pub_id = " . $ticket_row["focus_pub_id"];
$publications_result = mysqli_query($con, $sql_publications);

if ($publications_result) {
    $pub_row = mysqli_fetch_assoc($publications_result);
} else {
    echo "Error fetching publication details.";
}

$sql_users = "SELECT * FROM users WHERE user_id = " . $ticket_row["focus_user_id"];
...
if (isset($_POST['edit-pub-lending'])) {
  ...
  $observations = $_POST['observations'];
    ...
    if (isset($_POST["lend_status_manual"])) {
        // If it's set, assign its value to $lend_status_manual
        $lend_status_manual = $_POST["lend_status_manual"];
    } else {
        // If it's not set, you can initialize $lend_status_manual to a default value or leave it as null
        // For example:
        $lend_status_manual = null; // Or any default value you want to assign
    }
...
// Marcar el préstamo como activo si la fecha de inicio es la actual, o como presolicitud si aún no ha ocurrido, SI NO SE ESTÁ PONIENDO MANUALMENTE.

if (isset($lend_status_manual))
{
  $lend_status = $lend_status_manual;
}
else
{
  ...
  if (empty($errors)) {
   ...
   //Vamos a hacer todo en una transacción para que así sea más facil de procesar
    ...
    try {
        ...
        //Primera operación: Igual que en agregar, tenemos que registrar cualquier usuario ad_hoc.
        if ($ad_hoc == true)
        {
        ...
        // Primera modificación de verdad: Actualizar la publicación directamente
        $pub_lend_sql = "UPDATE pub_lending SET start_date = ?, end_date = ?, focus_user_id = ?, focus_pub_id = ?, observations = ?, lend_type = ?, lend_status = ?, quantity = ? WHERE book_act_id = ?";
        $pub_lend_stmt = mysqli_prepare($con, $pub_lend_sql);
        mysqli_stmt_bind_param($pub_lend_stmt, 'ssiissiii', $start_date, $end_date, $user_id, $pub_id, $observations, $lend_type, $lend_status, $quantity, $plc_id);
        mysqli_stmt_execute($pub_lend_stmt);
        mysqli_stmt_close($pub_lend_stmt);

        //Segundo paso: Eliminar todas las cotas en préstamo.
        $queryCotaDel = "DELETE FROM pub_lent_cotas WHERE pub_lend_id = " . $plc_id;
        mysqli_query($con, $queryCotaDel);

        //Tercer paso: Re-crear cotas nuevas en préstamo. 
          ...
          if (isset($cota))
          {
            foreach ($cota as $cota_row) {
              ...
              $pub_cotas_checked[] = "(" . $plc_id . ", " . $cota_row . ")";
            }
          }
     ...
     $success = "Préstamo editado exitósamente!";
     audit_log(AuditAction::PubLendEdit, $pub_id, $user_id, null);
        ...
        $errors[] = "Algo salió mal: " . $e->getMessage(); // Log the specific error message
            ...
            // Resacar detalles del ticket de la base de datos.
          $sql_tick = "SELECT * FROM pub_lending WHERE book_act_id = '$plc_id'";
          $tick_result = mysqli_query($con, $sql_tick);
          $ticket_row = mysqli_fetch_assoc($tick_result);
...
?>
<script>var isEditPage = true; //Esta variable existe para que el script de búsqueda corra en modo de editar página.</script>
...
<div class="card custom-wider-card">
  <div class="card-body">
    ...
    <form action="edit-pub-lending.php?id=<?php echo $plc_id; ?>" class="row g-3" method="post">
      <?php if ($ticket_row["lend_status"] > 0) { ?>
        <div class="form-group">
          <label for="lend_status_manual">Estado del Préstamo</label>
          <select class="form-control" id="lend_status_manual" name="lend_status_manual">
            <option value="01">Préstamo Activo</option>
            <option value="02">Préstamo Finalizado</option>
            <option value="03">Publicación Extravíada.</option>
          </select>
        </div>
      <?php } ?>
      <div class="col-md-6">
        ...
        <div class="form-group">
          ...
          <input type="date" class="form-control" id="start_date" name="start_date" placeholder="Fecha de inicio del préstamo." value="<?php echo $ticket_row['start_date']; ?>">
        </div>
      </div>
      ...
      <div class="col-md-6">
        ...
        <div class="form-group">
          ...
          <input type="date" class="form-control" id="end_date" name="end_date" placeholder="Fecha tope del préstamo." value="<?php echo $ticket_row['end_date']; ?>">
        </div>
      </div>
      ...
      <div class="col-md-6">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control" id="lend_name" name="lend_name" placeholder="Nombre del prestatario." autocomplete="off" value="<?php echo $user_row["username"]; ?>">
        </div>
    ...
    <div class="col-md-6">
        <div class="form-group">
          ...
          <input type="text" class="form-control" id="search_pub" name="search_pub" placeholder="Nombre de la publicación prestada." autocomplete="off" value="<?php echo $pub_row["name"];?>">
        </div>
    ...
    <div class="col-md-6">
      <div class="form-group">
        ...
        <div class="form-group">
          ...
          <textarea type="text" class="form-control" id="observations" name="observations" placeholder="Ingresar observaciones (opcional)." maxlength="1500"><?php echo $ticket_row["observations"]; ?></textarea>
        </div>

          <input type="hidden"  id="pub_id_hidden" name="pub_id">

      <div class="col-12 text-center mt-4">
        <button type="submit" class="btn btn-primary" name="edit-pub-lending">Editar</button>
      </div>
